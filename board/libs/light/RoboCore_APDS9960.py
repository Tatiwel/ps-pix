...
# RoboCore APDS-9960 Library (MicroPython) (v1.0.1)

# This library was originally created by @liske.
# ( https://github.com/RoboCore/RC-IDE_MicroPython_APDS-9960 )

# This is a version modified by Luan.F (22/05/2023) to be compatible with the RC-IDE. For that, it was necessary 
# to combine the files "const.py", "device.py" and "exceptions.py" (only the one used) in this file. 
# Other files were skipped.

# This file is part of the APDS-9960 library ("APDS-9960-lib").

# "APDS-9960-lib" is free software: you can redistribute it and/or modify
#  it under the terms of the GNU General Public License as published by
# the Free Software Foundation, either version 3 of the License, or
# (at your option) any later version.

# "APDS-9960-lib" is distributed in the hope that it will be useful,
# but WITHOUT ANY WARRANTY; without even the implied warranty of
# MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the
# GNU General Public License for more details.

# You should have received a copy of the GNU General Public License
# along with "APDS-9960-lib". If not, see <https://www.gnu.org/licenses/>
...
class APDS9960:
    ...
    def __init__(self, bus, address=APDS9960_I2C_ADDR, valid_id=APDS9960_DEV_ID):
        ...
        # created to control the activation of modes.
        self.modeProximity = False
        self.modeGestures = False
        self.modeAmbientLight = False
        ...
        if not self.dev_id in valid_id:
            raise Exception.__init__(self, "Device id 0x{} is not a valied one (valid: {})!".format(
                format(self.dev_id, '02x'), ', '.join(["0x{}".format(format(i, '02x')) for i in valid_id])))
        ...
        self._write_byte_data(APDS9960_REG_PPULSE,
                              APDS9960_DEFAULT_PROX_PPULSE)
        self._write_byte_data(APDS9960_REG_POFFSET_UR,
                              APDS9960_DEFAULT_POFFSET_UR)
        self._write_byte_data(APDS9960_REG_POFFSET_DL,
        ...
        self.setProximityGain(APDS9960_PGAIN_8X)
    ...
    def setMode(self, mode, enable=True):
        ...
        if mode < 0 or mode > APDS9960_MODE_ALL:
            raise Exception.__init__(
                self, "Feature mode {} is invalid!".format(mode))
        ...
        # ensures the writing of the register
        sleep(0.06) # [seg] (60 ms)

    # start the light (R/G/B/Ambient) sensor
    def enableLightSensor(self, interrupts=True):
        # Check if the modeAmbientLight variable is false
        if not self.modeAmbientLight:
            self.disableProximitySensor()  # Turn off proximity mode
            self.disableGestureSensor()  # Turn off gesture mode

            # self.setAmbientLightGain(APDS9960_DEFAULT_AGAIN)
            self.setAmbientLightIntEnable(interrupts)
            self.enablePower()
            self.setMode(APDS9960_MODE_AMBIENT_LIGHT, True)
            self.modeAmbientLight = True
    ...
    def enableProximitySensor(self, interrupts=True):
        # check if the modeProximity variable is false
        if not self.modeProximity:
            self.disableLightSensor()  # Turn off light mode
            self.disableGestureSensor()  # Turn off gesture mode

            self.setProximityIntEnable(interrupts)
            self.enablePower()
            self.setMode(APDS9960_MODE_PROXIMITY, True)
            self.modeProximity = True
    ...
    def enableGestureSensor(self, interrupts=True):
         # check if the modeGestures variable is false
        if not self.modeGestures:
            self.disableLightSensor()  # Turn off light mode
            self.disableProximitySensor()  # Turn off proximity mode (just to avoid interference)
    ...
    def readGesture(self):
        ...
        # enable gesture mode. The false serves to disable the module interrupt
        self.enableGestureSensor(False)
    ...
    def readAmbientLight(self):
        ...
        # enable light mode. The false serves to disable the module interrupt
        self.enableLightSensor(False)
    ...
    # read a color component (RGBC) of the light (0=ambient/clear, 1=red, 2=green, 3=blue)
    def readColor(self, color):
        
        # all components must be read for a proper reading of the sensor, but only one is returned
        ambientLight = self.readAmbientLight()
        redLight = self.readRedLight()
        greenLight = self.readGreenLight()
        blueLight = self.readBlueLight()
    ...
    def readProximity(self):
        ...
        # enable proximity mode. The false serves to disable the module interrupt
